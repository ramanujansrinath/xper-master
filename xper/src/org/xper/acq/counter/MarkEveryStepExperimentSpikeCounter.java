...
public class MarkEveryStepExperimentSpikeCounter {
	static Logger logger = Logger.getLogger(MarkEveryStepExperimentSpikeCounter.class);
	...
	protected MarkEveryStepTaskSpikeDataEntry getTaskSpike(TaskDoneEntry task,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Session: " + acq_session.getStartTime() + ", " + acq_session.getStopTime());
			for (AcqDataEntry e: acq_data) {
				System.out.println(e.getChannel() + " " + e.getSampleInd() + " " + e.getValue());
			}
		}
		
		if (acq_data.size() <= 0) {
			throw new InvalidAcqDataException("No AcqData find for task "
					+ task.getTaskId());
		}
